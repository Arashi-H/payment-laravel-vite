...
export const searchInObject= (array, searchVal) => {
    ...
    array.map(element => {
        ...
        const index = keys.findIndex(key => {
            if (typeof (element[key]) !== 'object' && key !== 'id' && `${element[key]}`.search(searchVal) !== -1) {
                return true
            } else if (typeof (element[key]) === 'object') {
                const idx = element[key].findIndex(elmnt => {
                    const subkeys = Object.keys(elmnt)
                    const subidx = subkeys.findIndex(subkey => typeof (elmnt[subkey]) !== 'object' && subkey !== 'id' && `${elmnt[subkey]}`.search(searchVal) !== -1)
                    return subidx >= 0
                })
                return idx >= 0
            }
